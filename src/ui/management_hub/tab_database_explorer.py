...
class DatabaseExplorerTab:
    ...
    def open_driver_profile(self, driver_id: int, return_subtab: Optional[str] = None, return_hub_tab: Optional[str] = None):
    ...
    def open_team_profile(self, team_id: int, team_tier: Optional[int] = None, return_subtab: Optional[str] = "DRIVERS"):
    ...
    def _get_filtered_drivers(self, gm: GameManager) -> List[Dict[str, Any]]:
        ...
        if self.driver_filter == "ALUMNI":
            ...
            return [d for d in gm.db.get_all_drivers_directory(player_team_id=gm.team_id) if (d.get("team_tier") in [4, 5] or d.get("is_academy_driver"))]
    ...
    def render(self, surface: pygame.Surface, gm: GameManager):
        ...
        for idx, (st_id, st_lbl) in enumerate(subtabs):
            ...
            pygame.draw.rect(surface, UITheme.ACCENT_CYAN if is_sel else UITheme.PANEL_BORDER, st_rect, width=1, border_radius=3)
    ...
    def _render_season_archive(self, surface: pygame.Surface, gm: GameManager):
        ...
        for t in range(1, 6):
            ...
            pygame.draw.rect(surface, UITheme.ACCENT_CYAN if is_sel else UITheme.PANEL_BORDER, t_btn, width=1, border_radius=2)
        ...
        pygame.draw.rect(surface, UITheme.ACCENT_CYAN if is_r else UITheme.PANEL_BORDER, r_btn, width=1, border_radius=2)
    ...
    def _render_archive_constructors_table(self, surface: pygame.Surface, gm: GameManager, box: pygame.Rect, season_num: int):
        ...
        surface.blit(self.font_badge.render("CONSTRUCTOR (CLICK FOR OVERVIEW)", True, UITheme.TEXT_MUTED), (th.x + 80, th.y + 5))
        ...
        surface.blit(self.font_badge.render("CHAMPIONSHIP POINTS", True, UITheme.TEXT_MUTED), (th.x + box.width - 210, th.y + 5))

        if not teams:
            surface.blit(self.font_body.render("No constructor records found for this season & tier.", True, UITheme.TEXT_MUTED), (box.x + 20, box.y + 45))
            return
        ...
        for idx, t in enumerate(teams):
            ...
            surface.blit(self.font_btn.render(t_name, True, (255, 215, 0) if is_ply else UITheme.TEXT_WHITE), (r_box.x + 80, r_box.y + 5))

            surface.blit(self.font_body.render(t.get("engine_supplier", "Standard"), True, UITheme.TEXT_MUTED), (r_box.x + 420, r_box.y + 6))
            surface.blit(self.font_body.render(f"{t.get('reputation', 50)} / 100", True, UITheme.TEXT_MUTED), (r_box.x + 650, r_box.y + 6))
    ...
    def _render_archive_drivers_table(self, surface: pygame.Surface, gm: GameManager, box: pygame.Rect, season_num: int):
        drivers = gm.db.get_historical_driver_standings(season_num, self.selected_tier)

        th = pygame.Rect(box.x + 8, box.y + 8, box.width - 16, 24)
        pygame.draw.rect(surface, (14, 18, 24), th)
        surface.blit(self.font_badge.render("POS", True, UITheme.TEXT_MUTED), (th.x + 12, th.y + 5))
        surface.blit(self.font_badge.render("DRIVER (CLICK FOR DOSSIER)", True, UITheme.TEXT_MUTED), (th.x + 80, th.y + 5))
        surface.blit(self.font_badge.render("CONSTRUCTOR (CLICK FOR OVERVIEW)", True, UITheme.TEXT_MUTED), (th.x + 360, th.y + 5))
        ...
        if not drivers:
            surface.blit(self.font_body.render("No driver records found for this season & tier.", True, UITheme.TEXT_MUTED), (box.x + 20, box.y + 45))
            return
        ...
        for idx, d in enumerate(drivers[:18]):
            ...
            surface.blit(self.font_body.render(str(wins), True, (255, 215, 0) if wins > 0 else UITheme.TEXT_MUTED), (r_box.x + 730, r_box.y + 5))
            surface.blit(self.font_body.render(str(pods), True, (0, 240, 140) if pods > 0 else UITheme.TEXT_MUTED), (r_box.x + 835, r_box.y + 5))

            pts = d.get("points", 0)
            surface.blit(self.font_btn.render(f"{pts} PTS", True, (0, 220, 255)), (r_box.x + box.width - 180, r_box.y + 4))
    ...
    def _render_archive_races_table(self, surface: pygame.Surface, gm: GameManager, box: pygame.Rect, season_num: int):
        ...
        surface.blit(self.font_badge.render("CLASSIFICATION", True, UITheme.TEXT_MUTED), (th.x + box.width - 220, th.y + 5))

        if not races:
            surface.blit(self.font_body.render("No race weekend logs found for this season & tier.", True, UITheme.TEXT_MUTED), (box.x + 20, box.y + 45))
            return
        ...
        for idx, r in enumerate(races):
            ...
            surface.blit(self.font_body_bold.render(r.get("track_name", "Circuit"), True, UITheme.TEXT_WHITE), (r_box.x + 90, r_box.y + 6))
            ...
            if w_drv:
                ...
                surface.blit(self.font_body.render(w_team or "", True, UITheme.TEXT_MUTED), (r_box.x + 680, r_box.y + 6))
                ...
                surface.blit(self.font_body.render("Upcoming Event / In Progress", True, UITheme.TEXT_MUTED), (r_box.x + 420, r_box.y + 6))
    ...
    def _render_drivers(self, surface: pygame.Surface, gm: GameManager):
        ...
        for f_key, f_lbl in filters:
            ...
            pygame.draw.rect(surface, (255, 215, 0) if (f_key == "ALUMNI" and is_sel) else (UITheme.ACCENT_CYAN if is_sel else UITheme.PANEL_BORDER), f_rect, width=1, border_radius=2)

            col = (255, 215, 0) if (f_key == "ALUMNI" and is_sel) else (UITheme.TEXT_WHITE if is_sel else UITheme.TEXT_MUTED)
        ...
        if not drivers:
            surface.blit(self.font_body.render("No drivers found matching this filter.", True, UITheme.TEXT_MUTED), (list_box.x + 16, list_box.y + 40))
        else:
            ...
            for idx, d in enumerate(drivers):
                ...
                surface.blit(self.font_btn.render(d["name"], True, (255, 215, 0) if is_alumni else UITheme.TEXT_WHITE), (r_box.x + 10, r_box.y + 4))

                t_tier = d.get("team_tier")
                t_name = d.get("team_name") or "Free Agent"
                tier_str = f"Tier {t_tier}" if t_tier else "Free Agent"
                surface.blit(self.font_body.render(f"{t_name} ({tier_str})", True, UITheme.TEXT_MUTED), (r_box.x + 10, r_box.y + 22))
                ...
                if is_alumni:
                    ...
                    surface.blit(self.font_badge.render("★ ALUMNI", True, (255, 215, 0)), (alumni_badge.x + 6, alumni_badge.y + 3))
                else:
                    pts = d.get("points", 0)
                    surface.blit(self.font_btn.render(f"{pts} PTS", True, (0, 220, 255)), (r_box.x + r_box.width - 65, r_box.y + 12))
        ...
        if self.selected_driver_id:
            ...
            surface.blit(self.font_body.render("Select a driver from the left directory to view full career dossier.", True, UITheme.TEXT_MUTED), (detail_box.x + 24, detail_box.y + 40))

    def _render_driver_dossier_content(self, surface: pygame.Surface, gm: GameManager, box: pygame.Rect, p: Dict[str, Any]):
        ...
        surface.blit(self.font_header.render(d_name.upper(), True, UITheme.TEXT_WHITE), (hdr_rect.x + 60, hdr_rect.y + 8))
        ...
        if team_id:
            ...
            surface.blit(self.font_body.render(meta_str, True, UITheme.TEXT_MUTED), (team_btn_rect.x + team_btn_rect.width + 12, hdr_rect.y + 28))
        else:
        ...
        if is_alumni:
            ...
            surface.blit(self.font_body_bold.render("★ OFFICIAL TEAM ALUMNI", True, (255, 215, 0)), (a_banner.x + 10, a_banner.y + 4))
        ...
        for idx, (stat_title, stat_val) in enumerate(stat_cards):
            ...
            val_col = (255, 215, 0) if "WIN" in stat_title or "CHAMPION" in stat_title else ((0, 220, 255) if "POINTS" in stat_title else UITheme.TEXT_WHITE)
        ...
        surface.blit(self.font_btn.render(f"🤖 AI PERFORMANCE REPORT  [{arc_label}]", True, (0, 220, 255)), (ai_box.x + 10, ai_box.y + 5))
        surface.blit(self.font_body.render(ai_data.get("scout_assessment", ""), True, UITheme.TEXT_WHITE), (ai_box.x + 10, ai_box.y + 23))

        alumni_note = ai_data.get("alumni_insight", "")
        if alumni_note:
            surface.blit(self.font_body.render(alumni_note, True, (255, 215, 0) if is_alumni else UITheme.TEXT_MUTED), (ai_box.x + 10, ai_box.y + 40))
        ...
        surface.blit(self.font_badge.render("POINTS", True, UITheme.TEXT_MUTED), (sh_rect.x + box.width - 110, sh_rect.y + 3))

        seasons_hist = p.get("season_history", [])
        if not seasons_hist:
            surface.blit(self.font_body.render("No historical season records on file.", True, UITheme.TEXT_MUTED), (box.x + 20, cur_y + 26))
        else:
            for s_idx, sh in enumerate(seasons_hist):
                row_y = cur_y + 24 + s_idx * 24
                ...
                s_yr = 2025 + s_num
                surface.blit(self.font_body_bold.render(f"Season {s_num} ({s_yr})", True, UITheme.TEXT_WHITE), (row_box.x + 8, row_box.y + 3))
                surface.blit(self.font_body.render(f"Tier {sh.get('tier', 3)}", True, UITheme.TEXT_MUTED), (row_box.x + 130, row_box.y + 3))
                ...
                surface.blit(self.font_body.render(str(sh.get("race_starts", 0)), True, UITheme.TEXT_MUTED), (row_box.x + 510, row_box.y + 3))
                surface.blit(self.font_body.render(str(sh.get("wins", 0)), True, (255, 215, 0) if sh.get("wins", 0) > 0 else UITheme.TEXT_MUTED), (row_box.x + 590, row_box.y + 3))
                surface.blit(self.font_body.render(str(sh.get("podiums", 0)), True, (0, 240, 140) if sh.get("podiums", 0) > 0 else UITheme.TEXT_MUTED), (row_box.x + 660, row_box.y + 3))
                surface.blit(self.font_body_bold.render(f"{sh.get('points', 0)} PTS", True, (0, 220, 255)), (row_box.x + box.width - 110, row_box.y + 3))
    ...
    def _render_constructors(self, surface: pygame.Surface, gm: GameManager):
        ...
        tier_names = {1: "TIER 1 (WSF)", 2: "TIER 2 (CONTINENTAL)", 3: "TIER 3 (NATIONAL)", 4: "TIER 4 (JUNIOR)", 5: "TIER 5 (KARTING)"}
        for t in range(1, 6):
            t_btn = pygame.Rect(24 + (t - 1) * 125, 108, 118, 26)
        ...
        if not teams:
            surface.blit(self.font_body.render("No teams found in this tier.", True, UITheme.TEXT_MUTED), (list_box.x + 14, list_box.y + 40))
        else:
            ...
            for idx, t in enumerate(teams):
                ...
                t_name = f"{t.get('name', 'Constructor')} {'[YOU]' if is_ply else ''}"
                surface.blit(self.font_btn.render(t_name, True, (255, 215, 0) if is_ply else UITheme.TEXT_WHITE), (r_box.x + 20, r_box.y + 4))

                eng = t.get("engine_supplier", "Standard")
                surface.blit(self.font_body.render(f"{eng}  |  Rep: {t.get('reputation', 50)}", True, UITheme.TEXT_MUTED), (r_box.x + 20, r_box.y + 24))
        ...
        if self.selected_team_id:
            ...
            surface.blit(self.font_body.render("Select a constructor from the left directory to view full overview.", True, UITheme.TEXT_MUTED), (detail_box.x + 24, detail_box.y + 40))

    def _render_constructor_dossier_content(self, surface: pygame.Surface, gm: GameManager, box: pygame.Rect, t: Dict[str, Any]):
        # Header Box
        hdr_rect = pygame.Rect(box.x, box.y, box.width, 50)
        pygame.draw.rect(surface, UITheme.PANEL_HEADER, hdr_rect, border_top_left_radius=4, border_top_right_radius=4)
        ...
        is_ply = bool(t.get("is_player"))
        surface.blit(self.font_header.render(f"{t_name.upper()} {'[YOUR TEAM]' if is_ply else ''}", True, (255, 215, 0) if is_ply else UITheme.TEXT_WHITE), (hdr_rect.x + 32, hdr_rect.y + 8))
        ...
        surface.blit(self.font_btn.render("CURRENT ACTIVE DRIVER LINEUP (CLICK DRIVER FOR DOSSIER)", True, UITheme.TEXT_WHITE), (box.x + 12, cur_y))
        cur_y += 20
        ...
        for d_idx in range(2):
            ...
            if d_idx < len(drivers):
                d = drivers[d_idx]
                surface.blit(self.font_badge.render(f"CAR #{d_idx+1}", True, UITheme.ACCENT_CYAN), (c_rect.x + 10, c_rect.y + 6))
                surface.blit(self.font_btn.render(f"#{d.get('number', 0)}  {d.get('name', 'Driver')}", True, (255, 215, 0) if d.get("is_player_driver") else UITheme.TEXT_WHITE), (c_rect.x + 10, c_rect.y + 20))
                surface.blit(self.font_body.render(f"Pace: {d.get('pace', 50)}  |  Age: {d.get('age', 25)}  |  {d.get('points', 0)} PTS", True, UITheme.TEXT_MUTED), (c_rect.x + 10, c_rect.y + 38))
                ...
                surface.blit(self.font_badge.render("DOSSIER 👤", True, UITheme.TEXT_WHITE), (view_btn.x + 10, view_btn.y + 5))
            else:
                surface.blit(self.font_badge.render(f"CAR #{d_idx+1} [SEAT VACANT]", True, UITheme.TEXT_MUTED), (c_rect.x + 10, c_rect.y + 20))
        ...
        for idx, (stat_title, stat_val) in enumerate(stat_cards):
            ...
            val_col = (255, 215, 0) if "WIN" in stat_title or "TITLE" in stat_title else ((0, 220, 255) if "POINTS" in stat_title else UITheme.TEXT_WHITE)
            v_surf = self.font_stat.render(stat_val, True, val_col)
        ...
        surface.blit(self.font_btn.render("HISTORICAL CHAMPIONSHIP RECORD", True, UITheme.TEXT_WHITE), (box.x + 12, cur_y))
        cur_y += 18
        ...
        surface.blit(self.font_badge.render("CHAMPIONSHIP FINISH", True, UITheme.TEXT_MUTED), (sh_rect.x + 300, sh_rect.y + 3))
        surface.blit(self.font_badge.render("TOTAL POINTS", True, UITheme.TEXT_MUTED), (sh_rect.x + box.width - 150, sh_rect.y + 3))

        hist = t.get("season_history", [])
        if not hist:
            surface.blit(self.font_body.render("No completed season records on file for this team.", True, UITheme.TEXT_MUTED), (box.x + 20, cur_y + 26))
        else:
            for s_idx, sh in enumerate(hist):
                ...
                s_yr = 2025 + s_num
                surface.blit(self.font_body_bold.render(f"Season {s_num} ({s_yr})", True, UITheme.TEXT_WHITE), (row_box.x + 8, row_box.y + 3))
                surface.blit(self.font_body.render(f"Tier {sh.get('tier', 3)}", True, UITheme.TEXT_MUTED), (row_box.x + 160, row_box.y + 3))

                pos = sh.get("championship_position", 10)
                pos_col = (255, 215, 0) if pos == 1 else ((0, 240, 140) if pos <= 3 else UITheme.TEXT_WHITE)
                surface.blit(self.font_badge.render(f"P{pos}", True, pos_col), (row_box.x + 300, row_box.y + 4))

                surface.blit(self.font_body_bold.render(f"{sh.get('points_total', 0)} PTS", True, (0, 220, 255)), (row_box.x + box.width - 150, row_box.y + 3))
    ...
    def _render_hall_of_fame(self, surface: pygame.Surface, gm: GameManager):
        ...
        quadrants = [
            ("🥇 ALL-TIME DRIVERS' CHAMPIONSHIPS (CLICK DRIVER)", records.get("top_champions", []), "titles", "TITLES", box.x + 12, box.y + 12),
            ("🏎️ ALL-TIME CONSTRUCTORS' TITLES (CLICK CONSTRUCTOR)", records.get("top_constructors", []), "titles", "TITLES", box.x + 24 + half_w, box.y + 12),
            ("🏁 MOST GRAND PRIX WINS (CLICK DRIVER)", records.get("top_wins_drivers", []), "total_wins", "WINS", box.x + 12, box.y + 24 + half_h),
            ("📊 ALL-TIME CAREER POINTS (CLICK DRIVER)", records.get("top_pts_drivers", []), "total_points", "POINTS", box.x + 24 + half_w, box.y + 24 + half_h)
        ]
        ...
        for q_title, q_list, val_key, val_unit, qx, qy in quadrants:
            ...
            if not q_list:
                surface.blit(self.font_body.render("No historical record entries on file.", True, UITheme.TEXT_MUTED), (q_box.x + 14, q_box.y + 40))
            else:
                for idx, entry in enumerate(q_list[:5]):
                    ...
                    surface.blit(self.font_btn.render(f"{val} {val_unit}", True, (255, 215, 0)), (q_box.x + q_box.width - 100, ry + 3))
    ...
    def _render_round_results_modal(self, surface: pygame.Surface):
        ...
        if not results:
            surface.blit(self.font_body.render("No classification records found for this race.", True, UITheme.TEXT_MUTED), (modal_x + 20, modal_y + 60))
            return
        ...
        surface.blit(self.font_badge.render("CONSTRUCTOR (CLICK TO VIEW)", True, UITheme.TEXT_MUTED), (th.x + 250, th.y + 3))
        ...
        for idx, r in enumerate(results[:12]):
            ...
            surface.blit(self.font_body.render(r.get("team_name", ""), True, (0, 220, 255)), (r_box.x + 250, r_box.y + 5))

            pts = r.get("points", 0)
            pts_col = (0, 220, 255) if pts > 0 else UITheme.TEXT_MUTED
            surface.blit(self.font_badge.render(f"+{pts} PTS" if pts > 0 else "-", True, pts_col), (r_box.x + 440, r_box.y + 5))
