...
BRAND_CATALOG = {
    "TITLE": [
        {
            "name": "Solaris Global Telecom",
            "color_hex": "#00d2be",
            "base_sign": 1200000,
            "base_per_race": 320000,
            "min_appeal": 65,
            "target_pos": 4,
            "target_bonus": 180000,
        },
        {
            "name": "Apex Hyper Energy",
            "color_hex": "#ff8700",
            "base_sign": 1500000,
            "base_per_race": 380000,
            "min_appeal": 75,
            "target_pos": 3,
            "target_bonus": 250000,
        },
        ...
        },
        {
            ...
            "color_hex": "#006f62",
            "base_sign": 1100000,
            "base_per_race": 290000,
            "min_appeal": 55,
            "target_pos": 5,
            "target_bonus": 150000,
        },
        {
            "name": "Horizon Microchips",
            "color_hex": "#1e41ff",
            "base_sign": 800000,
            "base_per_race": 210000,
            "min_appeal": 40,
            "target_pos": 7,
            "target_bonus": 100000,
        },
    ...
    "MIDDLE": [
        {
            ...
            "color_hex": "#b4ff00",
            "base_sign": 400000,
            "base_per_race": 110000,
            "min_appeal": 35,
            "target_pos": 8,
            "target_bonus": 60000,
        },
        {
            "name": "Castillo Petroleum",
            "color_hex": "#ffcc00",
            "base_sign": 500000,
            "base_per_race": 130000,
            "min_appeal": 45,
            "target_pos": 7,
            "target_bonus": 75000,
        },
        ...
        {
            "name": "Bavaria Automotive",
            "color_hex": "#3366cc",
            "base_sign": 450000,
            "base_per_race": 120000,
            "min_appeal": 50,
            "target_pos": 6,
            "target_bonus": 85000,
        },
        {
            "name": "Velocity Timepieces",
            "color_hex": "#cc3366",
            "base_sign": 300000,
            "base_per_race": 85000,
            "min_appeal": 10,
...
class SponsorManager:
    ...
    def calculate_sponsor_appeal(self, team_id: int) -> Dict[str, Any]:
        ...
        with self.db.get_connection() as conn:
            ...
            cur.execute(
                "SELECT finish_position FROM race_history WHERE team_id = ? ORDER BY id DESC LIMIT 10;", (team_id,)
            )
            ...
            cur.execute(
                "SELECT championship_position FROM season_history WHERE team_id = ? AND season_num >= 1 ORDER BY season_num DESC LIMIT 5;",
                (team_id,),
            )
            ...
            cur.execute(
                "SELECT AVG(marketability) FROM drivers WHERE team_id = ? AND is_academy_driver = 0;", (team_id,)
            )
            ...
            if museum_tier > 0:
                cur.execute(
                    "SELECT COUNT(*) FROM season_history WHERE team_id = ? AND championship_position = 1 AND season_num >= 1;",
                    (team_id,),
                )
    ...
    def get_active_sponsors(self, team_id: int) -> Dict[str, List[Dict[str, Any]]]:
        ...
        with self.db.get_connection() as conn:
            cur = conn.cursor()
            cur.execute(
                "SELECT * FROM active_sponsors WHERE team_id = ? ORDER BY slot_tier ASC, slot_index ASC;", (team_id,)
            )
    ...
    def _insert_offer(
        self, cur, team_id: int, slot_tier: str, template: Dict[str, Any], current_appeal: int, team_tier: int
    ):
    ...
    def seed_initial_sponsor_offers(self, team_id: int):
        ...
        with self.db.get_connection() as conn:
            ...
            mid_eligible = [
                b
                for b in BRAND_CATALOG["MIDDLE"]
                if b["min_appeal"] <= current_appeal + 8 and b["name"] not in used_brands
            ]
            if mid_eligible:
        ...
        with self.db.get_connection() as conn:
            ...
            for slot_tier, target in offer_targets.items():
                ...
                if len(tier_offers) < target:
                    eligible = [
                        b
                        for b in BRAND_CATALOG[slot_tier]
                        if b["min_appeal"] <= current_appeal + 8 and b["name"] not in used_brands
                    ]
                    if eligible:
    ...
    def sign_sponsor_offer(self, team_id: int, offer_id: int) -> Tuple[bool, str]:
        ...
        with self.db.get_connection() as conn:
            ...
            cur.execute(
                "SELECT COUNT(*) FROM active_sponsors WHERE team_id = ? AND slot_tier = ?;", (team_id, slot_tier)
            )
    ...
    def update_effective_team_name(self, team_id: int):
        ...
        with self.db.get_connection() as conn:
            ...
            cur.execute(
                "SELECT brand_name FROM active_sponsors WHERE team_id = ? AND slot_tier = 'TITLE' ORDER BY id ASC;",
                (team_id,),
            )
            title_sponsors = [r["brand_name"] for r in cur.fetchall()]

            cur.execute(
                "SELECT pay_driver_sponsor_name FROM drivers WHERE team_id = ? AND is_academy_driver = 0 AND driver_type = 'PAY_DRIVER' AND pay_driver_sponsor_name != '';",
                (team_id,),
            )
