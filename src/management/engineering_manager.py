...
FACTORY_PART_SPECS = {
    ...
    2: {
        ...
        "REAR_WING": {"name": "Continental High-Downforce Rear Wing", "cost": 350000.0, "perf": 96.0, "durability": 72.0},
    ...
    1: {
        ...
        "SUSPENSION": {"name": "Grand Prix Active Inerter Suspension", "cost": 1750000.0, "perf": 140.0, "durability": 80.0},
        "FLOOR": {"name": "Grand Prix Ground-Effect Diffuser Floor", "cost": 2400000.0, "perf": 150.0, "durability": 80.0},
...
RELEVANT_COMPONENT_FACILITIES = {
    "BRAKES": [
        "eng_brakes", "eng_thermal_rig", "mfg_cnc_machining", "mfg_additive_metal", 
        "eng_comp_materials", "test_qa_ndt", "eng_workshop"
    ],
    "FRONT_WING": [
        "eng_wings_front", "eng_windtunnel", "eng_cfd", "eng_aero_scanning", 
        "eng_aero_model_shop", "mfg_cleanroom_autoclave", "mfg_prepreg_freezer", 
        "mfg_rapid_tooling", "mfg_paint_bay", "test_qa_ndt", "eng_workshop"
    ],
    "REAR_WING": [
        "eng_wings_rear", "eng_windtunnel", "eng_cfd", "eng_aero_scanning", 
        "eng_aero_model_shop", "mfg_cleanroom_autoclave", "mfg_prepreg_freezer", 
        "mfg_rapid_tooling", "mfg_paint_bay", "test_qa_ndt", "eng_workshop"
    ],
    "FLOOR": [
        "eng_floor", "eng_windtunnel", "eng_cfd", "eng_aero_scanning", 
        "test_shaker_rig", "mfg_cleanroom_autoclave", "mfg_prepreg_freezer", 
        "eng_comp_materials", "test_qa_ndt", "eng_workshop"
    ],
    "SUSPENSION": [
        "eng_suspension", "eng_kinematics_lab", "test_shaker_rig", "test_torsional_rig", 
        "mfg_cnc_machining", "mfg_additive_metal", "mfg_monocoque_jig", 
        "eng_comp_materials", "test_qa_ndt", "eng_workshop"
    ],
    "ENGINE": [
        "eng_tuning", "eng_dyno", "eng_works_powertrain", "eng_thermal_rig", 
        "mfg_exotic_welding", "mfg_electronics", "mfg_additive_metal", 
        "test_qa_ndt", "eng_workshop"
    ],
    "ERS": [
        "eng_ers", "eng_dyno", "mfg_electronics", "test_qa_ndt", "eng_workshop"
    ]
}
...
class EngineeringManager:
    ...
    def get_facility_status_for_component(self, team_id: int, category: str) -> Tuple[bool, int, str]:
        ...
        with self.db.get_connection() as conn:
            ...
            cur.execute("SELECT is_unlocked, current_tier FROM team_facilities WHERE team_id = ? AND node_id = ?;", (team_id, fac_node))
            row = cur.fetchone()
    ...
    def process_post_race_telemetry(
        ...
        with self.db.get_connection() as conn:
            ...
            driver_factor = 0.75 + (driver_tech_skill / 100.0) * 0.35 + (driver_comm_skill / 100.0) * 0.25 + 0.05 * telemetry_rig_level + radio_factor
            ...
            for comp in components:
                ...
                fac_node_id = fac_node if cat != "ENGINE" else ("eng_works_powertrain" if works_tier > 0 else "eng_tuning")
                staff_out = self.staff_manager.calculate_facility_staff_output(team_id, fac_node_id, fac_tier, dev_gain_mult)
    ...
    def build_next_generation_part(self, team_id: int, component_id: int, cost_mult: float = 1.0) -> Tuple[bool, str, float]:
        """
        ...
        with self.db.get_connection() as conn:
            ...
            if comp["category"] == "ENGINE" and tier == 3:
                return False, f"Engine R&D is PROHIBITED in Tier 3 ({l_name}). Power units are customer engines contracted at season start.", 0.0

            if "SPEC" in allowed_parts or comp["category"] not in allowed_parts:
                return False, f"{comp['category']} development is SPEC REGULATED in Tier {tier} ({l_name}). Purchase Factory Supplier parts in the warehouse or earn promotion to unlock custom R&D!", 0.0

            # 2. Check Dedicated Facility Unlock (Must be Level >= 1)
            if comp["category"] == "ENGINE":
                if tier == 1:
                    cur.execute("SELECT is_unlocked, current_tier FROM team_facilities WHERE team_id = ? AND node_id = 'eng_works_powertrain';", (team_id,))
                    f_row = cur.fetchone()
                    if not f_row or not f_row[0] or f_row[1] < 1:
                        return False, "Cannot manufacture Works ENGINE. Construct Level 1 of Works Engine Factory in the Factory first.", 0.0
                else:
                    cur.execute("SELECT is_unlocked, current_tier FROM team_facilities WHERE team_id = ? AND node_id = 'eng_tuning';", (team_id,))
                    f_row = cur.fetchone()
                    if not f_row or not f_row[0] or f_row[1] < 1:
                        return False, "Cannot fine-tune ENGINE. Construct Level 1 of Engine Tuning Facility in the Factory first.", 0.0
            else:
                ...
                if fac_info:
                    ...
                    if not fac_row or not fac_row[0] or fac_row[1] < 1:
                        return False, f"Cannot manufacture {comp['category']} Mk {comp['generation'] + 1}. Construct Level 1 of {fac_name} in the Factory first.", 0.0
            ...
            fac_info = COMPONENT_FACILITY_MAP.get(comp["category"])
            fac_node_chk = fac_info[0] if fac_info else ("eng_tuning" if comp["category"] == "ENGINE" else "eng_workshop")
            cur.execute("SELECT current_tier, is_unlocked FROM team_facilities WHERE team_id = ? AND node_id = ?;", (team_id, fac_node_chk))
            ...
            cur_dur = comp["max_durability"] if "max_durability" in comp.keys() and comp["max_durability"] else {1: 80.0, 2: 72.0, 3: 65.0}.get(tier, 65.0)
            ...
            cur.execute("SELECT current_tier, is_unlocked FROM team_facilities WHERE team_id = ? AND node_id = 'test_qa_ndt';", (team_id,))
            ...
            dyno_extra = 0.0
            if comp["category"] == "ENGINE":
                cur.execute("SELECT current_tier, is_unlocked FROM team_facilities WHERE team_id = ? AND node_id = 'eng_dyno';", (team_id,))
            ...
            VALUES (?, 1, 'RND_BUILD', ?, ?);
            """, (team_id, f"Manufactured {comp['category']} Mk {new_gen} (+{gain:.1f} Perf, +{durability_gain:.1f}% Durability -> {new_max_durability:.0f}%)", -build_cost))
            
            conn.commit()
            return True, f"Successfully built {comp['category']} Mk {new_gen}! (+{gain:.1f} Perf, Durability upgraded to {new_max_durability:.0f}%).", gain
    ...
    def set_engine_supplier(self, team_id: int, supplier_name: str, force_new_season: bool = False) -> Tuple[bool, str]:
        ...
        with self.db.get_connection() as conn:
            cur = conn.cursor()
            cur.execute("SELECT tier, cash, engine_contract_races_left, engine_locked_for_season FROM teams WHERE id = ?;", (team_id,))
            res = cur.fetchone()
            ...
            if not force_new_season and is_locked and races_left > 0:
                return False, f"Engine contract is locked for this season ({races_left} races left). New supplier negotiations open at season end!"

            if supp.get("is_in_house", False):
                if tier != 1:
                    return False, "Works In-House Engine is only permitted in Tier 1 (World Super Formula)."
                cur.execute("SELECT is_unlocked FROM team_facilities WHERE team_id = ? AND node_id = 'eng_works_powertrain';", (team_id,))
                w_row = cur.fetchone()
    ...
    def build_facility_node(self, team_id: int, node_id: str, cost_mult: float = 1.0) -> Tuple[bool, str]:
        ...
        with self.db.get_connection() as conn:
            ...
            fac_req_tier = int(node["unlock_league_tier"] if "unlock_league_tier" in node.keys() and node["unlock_league_tier"] is not None else 3)
    ...
    def process_monthly_department_budgets(self, team_id: int, upkeep_mult: float = 1.0, cost_mult: float = 1.0) -> List[Dict[str, Any]]:
        """
        ...
        hr_lvl = (hr_fac["current_tier"] if hr_fac and hr_fac["is_unlocked"] else 0) + (headhunter_fac["current_tier"] if headhunter_fac and headhunter_fac["is_unlocked"] else 0)
        ...
        with self.db.get_connection() as conn:
            ...
            for fac in facilities:
                ...
                if surplus_deficit < 0:
                    ...
                    if needed_cut > 0:
                        cur.execute("SELECT id, salary_monthly FROM staff WHERE team_id = ? AND assigned_subnode = ? ORDER BY skill ASC;", (team_id, node_id))
                ...
                # =============================================================
                    ...
                    cur.execute("SELECT COUNT(*) FROM staff WHERE team_id = ? AND assigned_subnode = ?;", (team_id, node_id))
                    cur_staff = int(cur.fetchone()[0])
                    
                    if cur_staff < staff_cap and investable_surplus >= 4000.0:
                        # Auto-hire 1 specialized engineer
                        FIRST_NAMES = ["Alex", "James", "Elena", "Lucas", "Sophie", "Marcus", "Yuki", "Chloe", "David", "Liam"]
                        LAST_NAMES = ["Vance", "Kovacs", "Moreau", "Sato", "Schneider", "Ricci", "Novak", "Berg", "Dubois"]
                    ...
                    upgradeable_eq = [e for e in equipment_list if not e["is_tier_locked"] and e["current_level"] < e["max_level"]]
                    # Sort by highest ROI (perf bonus / cost)
                    upgradeable_eq.sort(key=lambda e: (e["perf_bonus_per_level"] / max(1.0, e["next_upgrade_cost"])), reverse=True)
    ...
    def process_weekly_next_gen_rnd(self, team_id: int, current_week: int) -> Dict[str, Any]:
        ...
        with self.db.get_connection() as conn:
            ...
            return {
                ...
                "bonus_rel_applied": bonus_applied,
    ...
    def execute_port_back_upgrade(self, team_id: int, current_week: int) -> Tuple[bool, str]:
        ...
        if regs.get("upcoming_package") != "STATUS_QUO":
            return False, f"Port-Back upgrades are prohibited because {regs.get('upcoming_package')} rule changes are announced for next year! Concepts cannot be adapted."
        ...
        pts = status["points"]
        if pts < 10000.0:
            return False, f"Insufficient next-gen progress to port back. Current progress: {pts:,.0f} pts (minimum 10,000 pts required)."
        ...
        with self.db.get_connection() as conn:
            ...
            """, (team_id, current_week, f"Ported Next-Gen Concept Back to Current Car (+{current_boost_add:.1f} Perf, +{current_rel_add:.1f}% Rel)"))

            conn.commit()

        return True, f"Port-Back Upgrade Successful! Current car received +{current_boost_add:.1f} Perf and +{current_rel_add:.1f}% Rel. Factory in 2-week retooling cooldown, followed by 3-week track testing telemetry bonuses."

    def evaluate_season_regulations(self, tier: int, current_week: int = 9, season_num: Optional[int] = None) -> Dict[str, Any]:
        """
        ...
        with self.db.get_connection() as conn:
            ...
            if is_ceiling_breach or is_mandatory_cycle or is_monopoly_runaway:
                if is_ceiling_breach:
                    ...
                    upcoming_pkg = random.choice(packages)
                    reason = f"Runaway championship gap detected (P1 lead over P2 > 35%)."
                ...
                announcement = f"FIA CONFIRMATION: Stable Technical Regulations (Status Quo) confirmed for next season! Competitive parity within acceptable safety margins."

            self.db.set_season_regulations(season_num, tier, upcoming_pkg, is_announced=True, announcement_text=announcement)
