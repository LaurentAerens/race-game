...
class GameManager:
    ...
    def get_financial_summary(self) -> Dict[str, Any]:
        ...
        with self.db.get_connection() as conn:
            ...
            cur.execute(
                "SELECT SUM(salary_per_race) FROM drivers WHERE team_id = ? AND is_academy_driver = 0;", (team_id,)
            )
            ...
            cur.execute(
                "SELECT SUM(sponsor_income_per_race) FROM drivers WHERE team_id = ? AND is_academy_driver = 0;",
                (team_id,),
            )
            ...
            cur.execute(
                "SELECT AVG(finish_position) FROM race_history WHERE team_id = ? ORDER BY id DESC LIMIT 5;", (team_id,)
            )
            rh_row = cur.fetchone()
            cur.execute(
                "SELECT championship_position FROM season_history WHERE team_id = ? ORDER BY id DESC LIMIT 1;",
                (team_id,),
            )
            sh_row = cur.fetchone()
            ...
            if "mkt_heritage" in comm_facs:
                ...
                cur.execute(
                    "SELECT COUNT(*) FROM season_history WHERE team_id = ? AND championship_position = 1;", (team_id,)
                )
    ...
    def process_weekly_cycle(self):
        ...
        with self.db.get_connection() as conn:
            ...
            for t in teams:
                ...
                if (
                    not t["is_player"]
                    and self.current_week >= 9
                    and float(t.get("next_gen_rnd_pct", 0.0) or 0.0) == 0.0
                ):
    ...
    def is_race_week_for_player(self) -> bool:
        ...
        with self.db.get_connection() as conn:
            cur = conn.cursor()
            cur.execute(
                "SELECT COUNT(*) FROM calendar WHERE tier = ? AND week = ? AND is_completed = 0;",
                (tier, self.current_week),
            )
    ...
    def get_other_series_racing_this_week(self) -> List[Dict[str, Any]]:
        ...
        with self.db.get_connection() as conn:
            cur = conn.cursor()
            cur.execute(
                "SELECT * FROM calendar WHERE week = ? AND tier != ? ORDER BY tier ASC;", (self.current_week, tier)
            )
    ...
    def get_current_race_event(self) -> Optional[Dict[str, Any]]:
        ...
        with self.db.get_connection() as conn:
            ...
            cur.execute(
                "SELECT * FROM calendar WHERE tier = ? AND is_completed = 0 ORDER BY round ASC LIMIT 1;", (tier,)
            )
            row = cur.fetchone()
            if row:
                return dict(row)
