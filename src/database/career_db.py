...
CORE_SPECIALTIES = [
    "BRAKES", "AERO_SURFACES", "FLOOR_UNDERBODY", "SUSPENSION",
    "POWERTRAIN_ICE", "ERS_HYBRID", "COMPOSITES", "CNC_MACHINING",
    "QA_DEFECT_SCREENING", "PIT_CREW_WHEELGUNS", "PIT_CREW_OPERATIONS",
    "RIVAL_INTELLIGENCE", "METEOROLOGY", "TELEMETRY_ANALYTICS",
    "MERCHANDISE", "PRESS_PR", "DIGITAL_MEDIA", "VIP_HOSPITALITY", "STAFF_HR",
    "DRIVER_COGNITIVE", "DRIVER_PHYSICAL", "DRIVER_PHYSIO", "RADIO_COMMS", "SCOUTING_YOUTH", "JUNIOR_DEVELOPMENT"
]
...
def seed_team_personnel(cur, team_id: int, tier: int, is_player: bool, unlocked_nodes: List[str]):
    ...
    if is_player:
        ...
        for node_id in unlocked_nodes:
            ...
            """, (
                team_id, node_id, h_name, h_age, h_age,
                random.randint(48, 54), random.randint(67, 75), fac_spec, h_salary, h_salary,
                h_skill if fac_spec in ["BRAKES", "AERO_SURFACES", "FLOOR_UNDERBODY", "SUSPENSION", "POWERTRAIN_ICE", "ERS_HYBRID"] else random.randint(25, 45),
                h_skill if fac_spec in ["COMPOSITES", "CNC_MACHINING", "QA_DEFECT_SCREENING", "PIT_CREW_WHEELGUNS"] else random.randint(25, 45),
        ...
        for _ in range(4):
            ...
            """, (
                team_id, role, i_name, i_age, i_age,
                random.randint(48, 54), i_spec, salary, salary,
                i_base, i_base, i_base, i_base,
                random.randint(25, 60), random.randint(25, 60), i_potential, 1 if is_intern else 0
            ))
    ...
    for cat in categories:
        ...
        if is_vacant:
            ...
            """, (
                team_id, cat, name, age, age,
                peak_age, retire_age, spec, salary, salary,
    ...
    for node_id in unlocked_nodes:
        ...
        """, (
            team_id, node_id, head_name, head_age, head_age,
            random.randint(48, 54), random.randint(67, 75), fac_spec, head_salary, head_salary,
            head_skill if fac_spec in ["BRAKES", "AERO_SURFACES", "FLOOR_UNDERBODY", "SUSPENSION", "POWERTRAIN_ICE", "ERS_HYBRID"] else random.randint(30, 55),
            head_skill if fac_spec in ["COMPOSITES", "CNC_MACHINING", "QA_DEFECT_SCREENING", "PIT_CREW_WHEELGUNS"] else random.randint(30, 55),
        ...
        for _ in range(num_staff):
            ...
            """, (
                team_id, node_id, s_name, s_age, s_age,
                random.randint(47, 53), random.randint(67, 72), s_spec, s_salary, s_salary,
                s_skill if s_spec in ["BRAKES", "AERO_SURFACES", "FLOOR_UNDERBODY", "SUSPENSION", "POWERTRAIN_ICE", "ERS_HYBRID"] else random.randint(25, 50),
                s_skill if s_spec in ["COMPOSITES", "CNC_MACHINING", "QA_DEFECT_SCREENING", "PIT_CREW_WHEELGUNS"] else random.randint(25, 50),
...
FIRST_NAMES = [
    "Lucas", "Marco", "Julian", "Alex", "Liam", "Carlos", "Nikita", "Arthur",
    "Oliver", "Felix", "Sebastian", "Gabriel", "Mateo", "Henrik", "Soren",
    "David", "Max", "Daniel", "Oscar", "Lando", "George", "Pierre", "Esteban",
    "Yuki", "Valtteri", "Fernando", "Sergio", "Lance", "Charles", "Lewis",
    "Kimi", "Kai", "Hugo", "Leo", "Theo", "Nico", "Jenson", "Mika", "Ayrton"
]
LAST_NAMES = [
    "Sterling", "Vance", "Moreno", "Tanaka", "Lindqvist", "Kovacs", "Rousseau",
    "Castillo", "De Vries", "Novak", "Sato", "Bergstrom", "Dupont", "Alvarez",
    "Fischer", "Nakamura", "Bianchi", "Kowalski", "Schneider", "Larsson",
    "Rossi", "Muller", "Webber", "Barrichello", "Hakkinen", "Prost", "Mansell",
    "Villeneuve", "Hill", "Coulthard", "Montoya", "Kubica", "Glock", "Trulli"
]

ALL_FACILITY_NODES = [
    # LAYER 0 (ROOT - TIER 3 STARTER GATEWAY)
    ("eng_workshop", "ENGINEERING", "R&D Workshop", "Base manufacturing shop for brakes & chassis", None, 1, 3, 2200000, 75000, 12, 1),

    # LAYER 1 (PRIMARY GATEWAYS & CORE DEPARTMENTS)
    # Tier 3 Gateways (~$1.8M - $2.2M):
    ("eng_brakes", "ENGINEERING", "Brakes Lab", "Caliper bite, cooling discs & lockup resistance", "eng_workshop", 1, 3, 1800000, 55000, 8, 1),
    ("eng_wings_front", "ENGINEERING", "Front Aero Lab", "Front wing cascade elements & ground clearance", "eng_workshop", 1, 3, 2200000, 65000, 10, 1),
    # Tier 2 Gateways (~$7M - $10M):
    ("eng_wings_rear", "ENGINEERING", "Rear Aero Lab", "Rear wing profiles, endplates & DRS flap channels", "eng_workshop", 1, 3, 7500000, 220000, 10, 1),
    ("eng_suspension", "ENGINEERING", "Suspension Shop", "Mechanical grip, curb ride & tire wear reduction", "eng_workshop", 1, 3, 8500000, 240000, 12, 1),
    ("eng_tuning", "ENGINEERING", "Engine Tuning", "ECU mapping & high-boost tuning calibration (+3.8 Perf, -0.5% Rel)", "eng_workshop", 1, 3, 9500000, 260000, 10, 1),
    ("eng_cad_office", "ENGINEERING", "CAD Design Office", "Structural stress FEA modeling & chassis packaging (+5% R&D Success)", "eng_workshop", 1, 3, 7000000, 200000, 10, 1),
    ("mfg_cleanroom_autoclave", "MANUFACTURING", "Autoclave Suite", "Carbon pre-preg layup & lightweight chassis (+1.5 Perf/part, -0.3% Rel)", "eng_workshop", 1, 3, 10500000, 300000, 14, 1),

    # LAYER 2 (SPECIALIZED LABS & SECONDARY GATEWAYS - TIER 2 TO TIER 1 JUMPS)
    ("eng_cfd", "ENGINEERING", "CFD Supercluster", "Virtual airflow simulation & aerodynamic flow analysis", "eng_cad_office", 1, 3, 14000000, 420000, 12, 1),
    ("eng_comp_materials", "ENGINEERING", "Materials Lab", "Microstructures, custom carbon weaves & exotic alloys (+1.5 Perf, +0.8% Rel)", "eng_cad_office", 1, 3, 11000000, 320000, 10, 1),
    ("eng_thermal_rig", "ENGINEERING", "Thermal Flow Rig", "Internal radiator airflow & brake duct thermal dissipation", "eng_brakes", 1, 3, 10000000, 290000, 10, 1),
    ("eng_aero_model_shop", "ENGINEERING", "Aero Model Shop", "Precision scale model workshop for wind tunnel verification", "eng_wings_rear", 1, 3, 9500000, 280000, 10, 1),
    ("eng_kinematics_lab", "TESTING", "Kinematics Rig", "Roll centers, camber gain & anti-dive motion simulation (-6% Tyre Deg)", "eng_suspension", 1, 3, 12500000, 360000, 12, 1),
    ("mfg_cnc_machining", "MANUFACTURING", "5-Axis CNC Shop", "Milling suspension uprights, wishbones & solid alloy parts", "eng_suspension", 1, 3, 11500000, 330000, 14, 1),
    ("mfg_prepreg_freezer", "MANUFACTURING", "Pre-Preg Freezers", "Sub-zero climate carbon roll storage & CNC ultrasonic ply cutting", "mfg_cleanroom_autoclave", 1, 3, 8500000, 240000, 10, 1),
    ("mfg_rapid_tooling", "MANUFACTURING", "Rapid Tooling", "High-speed dense foam routers creating master composite molds", "mfg_cleanroom_autoclave", 1, 3, 10000000, 280000, 12, 1),
    ("eng_dyno", "POWERTRAIN", "Engine Dyno Cells", "Transient dynamometers & stress testing (+1.8 Perf, +1.8% Rel)", "eng_tuning", 1, 3, 24000000, 650000, 16, 1),
    ("mfg_electronics", "MANUFACTURING", "Electronics Lab", "MIL-SPEC sealed looms, ECUs, sensor arrays & telemetry hardware", "eng_tuning", 1, 3, 9500000, 270000, 10, 1),

    # LAYER 3 (ADVANCED TESTING RIGS & ADDITIVE FABRICATION - TIER 1 PRE-APEX)
    ("eng_windtunnel", "ENGINEERING", "Wind Tunnel", "Advanced overarching aerodynamic testing for all aero surfaces (5 Parts)", "eng_wings_rear", 1, 3, 38000000, 950000, 18, 1),
    ("eng_aero_scanning", "ENGINEERING", "Aero PIV Scanner", "Particle Image Velocimetry & laser boundary layer mapping", "eng_cfd", 1, 3, 20000000, 550000, 12, 1),
    ("test_shaker_rig", "TESTING", "7-Post Shaker Rig", "Replicates track surface bumps, kerb strikes & aero heave (+15% Kerb Grip)", "eng_kinematics_lab", 1, 3, 22000000, 600000, 14, 1),
    ("mfg_additive_metal", "MANUFACTURING", "Metal 3D Printing", "Direct metal laser sintering of hollow titanium/Inconel components", "mfg_cnc_machining", 1, 3, 18000000, 500000, 12, 1),
    ("mfg_rapid_proto", "MANUFACTURING", "Rapid Prototyping", "Additive manufacturing of scale test parts (+8% Innovation Speed)", "mfg_cnc_machining", 1, 3, 11000000, 310000, 10, 1),
    ("mfg_monocoque_jig", "MANUFACTURING", "Monocoque Jig", "Ground-anchored steel alignment jigs for carbon safety cell bonding", "mfg_prepreg_freezer", 1, 3, 21000000, 580000, 14, 1),
    ("test_qa_ndt", "TESTING", "QA & NDT Lab", "Ultrasonic defect detection & failure prevention (+2.0% Rel, -0.4 Perf | 7 Parts)", "mfg_electronics", 1, 3, 9000000, 260000, 10, 1),
    ("mfg_exotic_welding", "MANUFACTURING", "Exotic Welding", "Argon-purged cleanrooms for paper-thin Inconel exhausts", "eng_dyno", 1, 3, 12000000, 340000, 10, 1),
    ("eng_ers", "POWERTRAIN", "ERS Hybrid Lab", "High-voltage battery pack assembly, MGU-K/MGU-H maps (Accessible entry-level)", "eng_workshop", 1, 3, 9500000, 260000, 12, 1),
    ...
    ("eng_floor", "ENGINEERING", "Underbody Lab", "Underbody Venturi tunnels & ground effect downforce (Level 1 Starter)", "eng_workshop", 1, 3, 8000000, 220000, 12, 1),
    ("mfg_paint_bay", "MANUFACTURING", "Paint & Livery Bay", "Specialized boundary-layer topcoats & lightweight liveries (+0.5 Perf, -0.2% Rel)", "eng_workshop", 1, 3, 7500000, 210000, 8, 1),

    ("test_torsional_rig", "TESTING", "Torsional Rig", "Hydraulic load frames twisting bare tubs for structural stiffness", "mfg_monocoque_jig", 1, 3, 16000000, 440000, 12, 1),
    ("eng_works_powertrain", "POWERTRAIN", "Works Engine Lab", "Full bespoke V6 Turbo & Works Power Unit builder ($0 Supplier Fee)", "eng_tuning", 1, 3, 85000000, 2400000, 35, 1),


    # HR & WORKFORCE AUTOMATION SUITE
    ("hr_recruitment", "HR", "Recruitment Bureau", "T1: Auto-Fill Desks | T2: Auto-Intern Pipeline with Min-Wage Budget Check", None, 1, 2, 4500000, 130000, 6, 1),
    ("hr_headhunting", "HR", "Executive Headhunting", "Autonomous rival paddock scouting & best-value talent poaching", "hr_recruitment", 1, 3, 8500000, 240000, 8, 1),
    ("hr_payroll", "HR", "Payroll Calibration", "Automated market wage adjustments within budget to prevent discontent", "hr_recruitment", 1, 3, 6000000, 170000, 6, 1),
    ("hr_teambuilding", "HR", "Culture & Welfare", "Buffers underpaid wage discontent (25-35%) & accelerates morale recovery", "hr_recruitment", 1, 3, 9000000, 260000, 8, 1),
    ("hr_leadership_institute", "HR", "Leadership Institute", "Accelerates weekly Leadership & Communication growth for Heads & Directors", "hr_recruitment", 1, 3, 12000000, 340000, 10, 1),
    ("hr_performance_review", "HR", "Performance Analytics", "Fog-of-War Stat Clarity: Narrows stat uncertainty range to exact numbers", "hr_recruitment", 1, 3, 10500000, 300000, 10, 1),
    ("hr_performance_cull", "HR", "Talent Exit Review", "Demographic age-curve review: auto-dismisses severe underperformers", "hr_performance_review", 1, 3, 7500000, 210000, 8, 1),
    ("hr_tech_academy", "HR", "Tech R&D Academy", "Weekly training gains for Engineering stat & potential conversion", "hr_performance_review", 1, 3, 16000000, 440000, 12, 1),
    ("hr_craft_workshop", "HR", "Craft Guild", "Weekly training gains for Craftsmanship & Composure for mfg/testing techs", "hr_performance_review", 1, 3, 14000000, 390000, 10, 1),
    ("hr_workforce_optimizer", "HR", "Succession Optimizer", "Auto-replaces staff when strictly superior talent is available for same/lower wage", "hr_performance_cull", 1, 3, 19000000, 520000, 12, 1),
    ("hr_wellness_center", "HR", "Longevity Center", "Extends peak age to 54 & reduces post-50 stat degradation by 60%", "hr_teambuilding", 1, 3, 13500000, 380000, 10, 1),
    ("hr_equipment_procurement", "HR", "Rig Procurement", "Uses accumulated Department Savings to auto-buy/upgrade equipment rigs", "hr_payroll", 1, 3, 11000000, 310000, 10, 1),


    # COMMERCIAL & MARKETING
    ("mkt_press", "COMMERCIAL", "Press & PR Office", "Media access, press releases & sentiment protection (+4 Marketability)", None, 1, 3, 2400000, 70000, 6, 1),
    ("mkt_brand_design", "COMMERCIAL", "Brand & Livery", "Livery styling & decal placement (+6 Marketability, +5% Sponsor Value)", "mkt_press", 1, 3, 8500000, 240000, 8, 1),
    ("mkt_digital", "COMMERCIAL", "Digital & Social", "Real-time coverage & viral fan channels (+8 Marketability)", "mkt_press", 1, 3, 7800000, 220000, 8, 1),
    ("mkt_merch", "COMMERCIAL", "Merchandise", "Official apparel & fan gear (+5 Marketability & performance-scaled retail sales)", "mkt_brand_design", 1, 3, 11500000, 320000, 10, 1),
    ("mkt_fan_club", "COMMERCIAL", "Fan Club", "Fan community & loyalty voting (+5 Marketability & performance-scaled dues)", "mkt_digital", 1, 3, 9500000, 270000, 8, 1),
    ("mkt_studio", "COMMERCIAL", "Media Studio", "Soundstage, car launches & docuseries (+12 Marketability)", "mkt_digital", 1, 3, 21000000, 580000, 12, 1),
    ("mkt_hospitality", "COMMERCIAL", "VIP Hospitality", "Paddock Club suites & executive lounges (+10 Sponsor Appeal)", "mkt_brand_design", 1, 3, 29000000, 780000, 14, 1),
    ("mkt_licensing", "COMMERCIAL", "Brand Licensing", "Gaming CAD & diecast partnerships (+6 Marketability & licensing royalties)", "mkt_merch", 1, 3, 16500000, 450000, 10, 1),
    ("mkt_esports", "COMMERCIAL", "Esports Rig", "Factory sim racing team & streaming rigs (+6 Marketability)", "mkt_studio", 1, 3, 13500000, 380000, 10, 1),
    ("mkt_heritage", "COMMERCIAL", "Heritage Museum", "Championship trophy showroom (+8 Marketability + 4 pts/Title & museum income)", "mkt_licensing", 1, 3, 25000000, 680000, 12, 1),
    ("mkt_customer_racing", "COMMERCIAL", "Customer Racing", "Retired F1 chassis sales & VIP client track days (+15 Sponsor Appeal & sales)", "mkt_hospitality", 1, 3, 65000000, 1750000, 18, 1),


    # TRACKSIDE & PIT CREW
    ("track_pitrig", "TRACKSIDE", "Pit Practice Rig", "Basic 12-man pit crew reaction training", None, 1, 3, 2200000, 68000, 14, 1),
    ("track_wheelguns", "TRACKSIDE", "Carbon Guns", "2.1s pit stops & laser wheel-nut alignment", "track_pitrig", 1, 3, 12500000, 360000, 18, 1),
    ("track_telemetry", "TRACKSIDE", "Track Telemetry", "High-accuracy live weather & race telemetry", "track_pitrig", 1, 3, 9800000, 280000, 8, 1),
    ("track_fast_repair", "TRACKSIDE", "Rapid Repair Gantry", "Sub-2.5s wing swaps & rapid composite bonding (-50% repair box time)", "track_wheelguns", 1, 3, 13500000, 390000, 12, 1),
    ("track_jack_release", "TRACKSIDE", "Active Jack & Release", "Sub-0.2s pneumatic lifting & automated green-light traffic gantry (Sub-2.0s stops)", "track_fast_repair", 1, 3, 21000000, 580000, 14, 1),
    ("track_rival_intel", "TRACKSIDE", "Paddock Recon Unit", "Trackside rival intelligence, acoustic listening & +40% competitor pitch roll rate", "track_telemetry", 1, 3, 11500000, 330000, 10, 1),
    ("track_reverse_eng", "TRACKSIDE", "Optical Telemetry Intercept", "Pit-straight dynamic LIDAR ride height profiling & +35% reverse-engineering knowledge", "track_rival_intel", 1, 3, 19000000, 520000, 12, 1),
    ("track_weather_station", "TRACKSIDE", "Doppler Weather Radar", "Dual-polarization mobile radar extending race rain forecast to 16 laps", "track_telemetry", 1, 3, 12000000, 350000, 8, 1),
    ("track_setup_telemetry", "TRACKSIDE", "Setup Analytics", "Dynamic ride height & damper telemetry providing recommended setup ranges in FP", "track_telemetry", 1, 3, 15000000, 420000, 10, 1),
    ("track_virtual_sim", "TRACKSIDE", "Virtual FP Solver", "10,000 synthetic pre-weekend laps ensuring FP1 starts significantly closer to optimal", "track_setup_telemetry", 1, 3, 26000000, 720000, 12, 1),
    ("track_comm_uplink", "TRACKSIDE", "Factory Mission Control", "10,000Hz satellite telemetry uplink boosting post-race knowledge & reliability on all parts", "track_telemetry", 1, 3, 22000000, 600000, 12, 1),

    # DRIVER PERFORMANCE & ACADEMY
    ("driver_sim", "DRIVER_PERF", "Driver Sim Rig", "Driver track familiarity & baseline training", None, 1, 3, 2300000, 72000, 6, 1),
    ("driver_motion_sim", "DRIVER_PERF", "Hexapod Sim", "Driver-in-the-Loop Hexapod Sim (+50% driver XP)", "driver_sim", 1, 3, 26000000, 720000, 10, 1),
    ("driver_vr_cognitive", "DRIVER_PERF", "Neuro-Reflex Lab", "Batak reaction matrix & saccade tracking (+30% Race Starts, Defending & Consistency)", "driver_motion_sim", 1, 3, 13500000, 390000, 8, 1),
    ("driver_gym_conditioning", "DRIVER_PERF", "Biometric Gym", "G-force neck harnesses & sauna heat chambers (+25% Tire Management & Wet Weather)", "driver_sim", 1, 3, 9500000, 275000, 8, 1),
    ("driver_physio_recovery", "DRIVER_PERF", "Physio Clinic", "Cryotherapy & hyperbaric recovery (-35% Age 30+ physical decline decay)", "driver_gym_conditioning", 1, 3, 17000000, 480000, 10, 1),
    ("driver_media_pr_coach", "DRIVER_PERF", "Media & PR Studio", "Paddock press simulation & crisis coaching (+35% Marketability & morale shield)", "driver_sim", 1, 3, 10500000, 300000, 8, 1),
    ("driver_radio_comms_lab", "DRIVER_PERF", "Radio Comms Lab", "115dB cockpit acoustic simulation & telemetry debrief (+35% Comms & +15% R&D insight)", "driver_media_pr_coach", 1, 3, 14000000, 400000, 10, 1),
    ("driver_commercial_suite", "DRIVER_PERF", "Sponsor Suite", "Brand ambassador stage & VIP lounges (+5 Sponsor Appeal & +6% race bonus)", "driver_media_pr_coach", 1, 3, 18500000, 520000, 12, 1),
    ("driver_academy", "DRIVER_PERF", "Driver Academy", "Scout, sign & fund junior drivers in feeder leagues", "driver_sim", 1, 3, 16000000, 460000, 12, 1),
    ("driver_karting_scholarship", "DRIVER_PERF", "Karting Foundation", "Grassroots talent radar in Tier 5 KMA (Guaranteed prodigy & +4 min potential floor)", "driver_academy", 1, 3, 19000000, 540000, 12, 1),
    ("driver_f4_bootcamp", "DRIVER_PERF", "Junior Boot Camp", "Single-seater test fleet & telemetry track camp (+35% Feeder Driver XP & smooth promotion)", "driver_karting_scholarship", 1, 3, 24000000, 680000, 14, 1),

    # MANAGEMENT (SINGLE-NODE FACTORY LEADERSHIP ENGINE)
    ("mgmt_boardroom", "MANAGEMENT", "Executive Boardroom", "Executive leadership command center improving overall leadership, coordination, and synergy for all factory personnel (+5 to +15 Global Leadership)", None, 1, 3, 2600000, 78000, 8, 1)
]
...
class CareerDatabase:
    ...
    def init_schema(self):
        ...
        with self.get_connection() as conn:
            ...
            for col_name, col_type in [("max_durability", "REAL NOT NULL DEFAULT 100.0"), ("current_durability", "REAL NOT NULL DEFAULT 100.0")]:
                try:
            ...
            cur.execute("CREATE INDEX IF NOT EXISTS idx_series_race_results_season ON series_race_results(season_num, tier, driver_id);")
            ...
            cur.executemany("INSERT OR REPLACE INTO facility_equipment VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);", EQUIPMENT_CATALOG)
            ...
            for t_row in existing_teams:
                ...
                if p_cnt == 0:
                    ...
                    if not u_nodes:
                        u_nodes = ["eng_workshop", "eng_brakes", "eng_wings_front", "mkt_press", "track_pitrig", "driver_sim", "mgmt_boardroom"]
    ...
    def _seed_default_data(self):
        ...
        with self.get_connection() as conn:
            ...
            # Seed 5 Leagues
            leagues_data = [
                (1, "World Super Formula", "WSF", 1, 1, 200000000, 3.5, "BRAKES,REAR_WING,FRONT_WING,SUSPENSION,ENGINE,FLOOR,ERS"),
                (2, "Continental Championship", "CC", 2, 1, 60000000, 2.0, "BRAKES,REAR_WING,FRONT_WING,SUSPENSION,ENGINE"),
            ...
            for tier, teams_list in TIER_TEAMS.items():
                for t_idx, t_data in enumerate(teams_list):
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?);
                    """, (t_data["name"], t_data["name"], t_data["color_hex"], tier, 1 if is_player else 0, t_data["budget"], t_data["reputation"]))
                    
                    team_id = cur.lastrowid

                    # Unlock base facilities for this team (No HR unlocked at start)
                    base_nodes = ["eng_workshop", "eng_brakes", "eng_wings_front", "mkt_press", "track_pitrig", "driver_sim", "mgmt_boardroom"]
                    if tier <= 2:
                        base_nodes.extend(["eng_wings_rear", "eng_windtunnel", "eng_suspension", "eng_tuning", "eng_cfd", "eng_cad_office", "track_wheelguns", "track_telemetry", "driver_motion_sim"])
                    if tier == 1:
                        base_nodes.extend(["eng_floor", "eng_ers", "eng_dyno", "eng_works_powertrain", "mfg_cleanroom_autoclave", "mfg_cnc_machining", "test_shaker_rig", "test_qa_ndt", "mkt_hospitality", "driver_academy", "hr_recruitment", "track_fast_repair", "track_setup_telemetry"])
                    ...
                    for node_id in base_nodes:
                        cur.execute("SELECT SUM(salary_monthly) FROM personnel WHERE team_id = ? AND facility_node_id = ?;", (team_id, node_id))
                        staff_sal = float(cur.fetchone()[0] or 0.0)
                        cur.execute("SELECT base_upkeep FROM facility_nodes WHERE id = ?;", (node_id,))
                        base_upk = float(cur.fetchone()[0] or 0.0)
                        min_init_budget = base_upk + staff_sal
                        cur.execute("UPDATE team_facilities SET monthly_sub_budget = MAX(monthly_sub_budget, ?) WHERE team_id = ? AND node_id = ?;", (min_init_budget, team_id, node_id))
                    ...
                    for d_slot in driver_slots:
                        ...
                        if is_player:
                            ...
                            """, (
                                team_id, d_name, age, d_num, 1 if d_slot == 1 else 0,
                                base_val, base_val, base_val, base_val, base_val, base_val,
                                base_val, base_val, base_val, base_val, base_val
                            ))
                        else:
                            ...
                            """, (
                                team_id, d_name, age, d_num,
                                (base_val * 1200) if tier <= 3 else 1000,
                                min(99, base_val + random.randint(3, 16) if age < 24 else base_val + 2),
                                base_val + random.randint(-3, 3), base_val + random.randint(-3, 3),
                                base_val + random.randint(-2, 4), base_val + random.randint(-4, 3),
                                base_val + random.randint(-3, 3), base_val + random.randint(-3, 3),
                                base_val + random.randint(-4, 4), base_val + random.randint(-5, 5),
                                base_val + random.randint(-4, 4), base_val + random.randint(-3, 3),
                                base_val + random.randint(-5, 5),
                                min(99, base_val+6), min(99, base_val+6), min(99, base_val+6),
                                min(99, base_val+6), min(99, base_val+6), min(99, base_val+6),
                                min(99, base_val+6), min(99, base_val+6), min(99, base_val+8),
                                min(99, base_val+8), min(99, base_val+8)
                            ))
    ...
    def _migrate_calendar_weeks(self, cur: sqlite3.Cursor):
        ...
        week_mappings = {
            ...
            (1, 9): 10, (1, 10): 11, (1, 11): 12, (1, 12): 13, (1, 13): 14, (1, 14): 15, (1, 15): 16, (1, 16): 18,
            (2, 1): 1, (2, 2): 2, (2, 3): 4, (2, 4): 5, (2, 5): 7, (2, 6): 8, (2, 7): 10, (2, 8): 11,
            (2, 9): 13, (2, 10): 14, (2, 11): 16, (2, 12): 18,
            (3, 1): 1, (3, 2): 3, (3, 3): 6, (3, 4): 9, (3, 5): 12, (3, 6): 15, (3, 7): 18,
            (4, 1): 2, (4, 2): 5, (4, 3): 8, (4, 4): 11, (4, 5): 14, (4, 6): 17,
            (5, 1): 1, (5, 2): 4, (5, 3): 7, (5, 4): 10, (5, 5): 13, (5, 6): 16
        }
    ...
    def get_series_race_results(self, tier: int, round_num: int, season_num: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    def award_driver_champion(self, driver_id: int) -> Dict[str, Any]:
        ...
        with self.get_connection() as conn:
            ...
            if driver.get("team_is_player") or driver.get("is_player_driver"):
                ...
                cur.execute("UPDATE teams SET cash = cash + ?, reputation = MIN(100, reputation + 8) WHERE id = ?;", (royalty_awarded, driver["team_id"]))
                cur.execute("""
                INSERT INTO ledger (team_id, week, category, description, amount)
                VALUES (?, 18, 'SPONSOR', ?, ?);
                """, (driver["team_id"], f"Driver Championship Merchandising Royalty ({driver['name']})", royalty_awarded))
    ...
    def get_season_regulations(self, tier: int, season_num: Optional[int] = None) -> Dict[str, Any]:
        ...
        with self.get_connection() as conn:
            ...
            cur.execute("SELECT consecutive_stable_seasons FROM season_regulations WHERE season_num = ? AND tier = ?;", (season_num - 1, tier))
            prev = cur.fetchone()
    ...
    def set_season_regulations(self, season_num: int, tier: int, upcoming_package: str, is_announced: bool = True, announcement_text: str = ""):
    ...
    def get_career_summary(self) -> Optional[Dict[str, Any]]:
        ...
        with self.get_connection() as conn:
            ...
            cur.execute("SELECT name, number FROM drivers WHERE team_id = ? AND is_academy_driver = 0 LIMIT 2;", (p_team["id"],))
    ...
    def get_team_components(self, team_id: int) -> List[Dict[str, Any]]:
        ...
        with self.get_connection() as conn:
            cur = conn.cursor()
            cur.execute("SELECT * FROM car_components WHERE team_id = ? ORDER BY car_slot ASC, category ASC;", (team_id,))
            return [dict(r) for r in cur.fetchall()]
    ...
    def get_team_warehouse_components(self, team_id: int, category: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        with self.get_connection() as conn:
            cur = conn.cursor()
            if category:
                cur.execute("SELECT * FROM car_components WHERE team_id = ? AND car_slot = 0 AND category = ? ORDER BY generation DESC, performance DESC;", (team_id, category))
            else:
                cur.execute("SELECT * FROM car_components WHERE team_id = ? AND car_slot = 0 ORDER BY category ASC, generation DESC;", (team_id,))
            return [dict(r) for r in cur.fetchall()]

    def buy_factory_component(self, team_id: int, category: str, cost: float, performance: float, durability: float, target_car_slot: int = 0) -> Tuple[bool, str, Optional[Dict[str, Any]]]:
        ...
        with self.get_connection() as conn:
            ...
            if target_car_slot in (1, 2):
                cur.execute("UPDATE car_components SET car_slot = 0 WHERE team_id = ? AND car_slot = ? AND category = ?;", (team_id, target_car_slot, category))
            ...
            return True, f"Factory {category} purchased for ${cost:,.0f} and placed into {dest} ({durability:.0f}% Durability).", row
    ...
    def consume_spare_component(self, team_id: int, category: str) -> Optional[Dict[str, Any]]:
        ...
        with self.get_connection() as conn:
            cur = conn.cursor()
            cur.execute("SELECT * FROM car_components WHERE team_id = ? AND car_slot = 0 AND category = ? ORDER BY current_durability DESC, generation DESC LIMIT 1;", (team_id, category))
            row = cur.fetchone()
            if not row:
                return None
            comp = dict(row)
    ...
    def get_facility_equipment(self, team_id: int, node_id: str, upkeep_mult: float = 1.0) -> List[Dict[str, Any]]:
        ...
        with self.get_connection() as conn:
            ...
            for r in cur.fetchall():
                ...
                d["current_upkeep"] = (d["base_upkeep"] * d["current_level"] * upkeep_mult) if (d["is_active"] and d["current_level"] > 0) else 0.0
    ...
    def get_department_financial_status(self, team_id: int, node_id: str, upkeep_mult: float = 1.0, conn: Optional[Any] = None) -> Dict[str, Any]:
    ...
    def get_facility_savings(self, team_id: int, node_id: str) -> float:
        ...
        with self.get_connection() as conn:
            cur = conn.cursor()
            cur.execute("SELECT savings_balance FROM team_facilities WHERE team_id = ? AND node_id = ?;", (team_id, node_id))
            row = cur.fetchone()
    ...
    def deposit_facility_savings(self, team_id: int, node_id: str, amount: float):
        ...
        with self.get_connection() as conn:
            cur = conn.cursor()
            cur.execute("UPDATE team_facilities SET savings_balance = savings_balance + ? WHERE team_id = ? AND node_id = ?;", (amount, team_id, node_id))
            conn.commit()

    def sweep_facility_savings_to_treasury(self, team_id: int, node_id: str, amount: Optional[float] = None) -> Tuple[bool, str, float]:
        ...
        with self.get_connection() as conn:
            ...
            cur.execute("UPDATE team_facilities SET savings_balance = savings_balance - ? WHERE team_id = ? AND node_id = ?;", (sweep_amt, team_id, node_id))
    ...
    def update_hr_policy(self, team_id: int, policy_key: str, value: Any):
        ...
        allowed_keys = [
            "auto_fill_desks", "auto_intern_pipeline", "auto_headhunt", "auto_payroll",
            "auto_cull", "auto_replace", "auto_equip_procure", "min_intern_potential",
    ...
    def process_monthly_department_savings(self, team_id: int, upkeep_mult: float = 1.0):
        ...
        with self.get_connection() as conn:
            ...
            for n in unlocked:
                ...
                if surplus > 0:
                    cur.execute("UPDATE team_facilities SET savings_balance = savings_balance + ? WHERE team_id = ? AND node_id = ?;", (surplus, team_id, n))
            conn.commit()
    ...
    def create_new_career(self, player_team_name: str, color_hex: str, difficulty: str = "NORMAL", engine_supplier: str = "Vortex EcoTech", principal_name: str = "Alex Mercer", enable_tutorial: bool = True):
        ...
        tables = [
            "teams", "drivers", "staff", "personnel", "team_category_directors", "personnel_applications", "team_hr_policies",
            "car_components", "team_facilities", "facility_nodes", 
            "facility_equipment", "team_equipment", "leagues", "active_sponsors", "sponsor_offers", 
            "race_history", "season_history", "calendar", "ledger", "innovation_pitches", "scout_prospects",
            "series_race_results", "driver_season_history", "season_regulations", "driver_market", "feeder_market_seats", "team_alumni", "balance_settings",
            "tutorial_progress"
        ]
        ...
        diff_cash_bonuses = {"VERY_EASY": 5000000.0, "EASY": 2500000.0, "NORMAL": 0.0, "HARD": -2000000.0, "VERY_HARD": -4000000.0}
        ...
        with self.get_connection() as conn:
            ...
            WHERE is_player = 1;
            """, (player_team_name, player_team_name, color_hex, cash_bonus, contract_cost, engine_supplier, contract_cost, difficulty, principal_name))
    ...
    def save_tutorial_progress(self, team_id: int, step_id: str, is_active: bool, is_completed: bool, is_skipped: bool, **kwargs):
        ...
        with self.get_connection() as conn:
            ...
            for key in ["brakes_equipment_bought", "personnel_hired", "front_wing_built", "driver_placed", "sponsor_signed"]:
                if key in kwargs:
                    cur.execute(f"UPDATE tutorial_progress SET {key} = ? WHERE team_id = ?;", (1 if kwargs[key] else 0, team_id))
            conn.commit()
    ...
    def add_team_cash(self, team_id: int, amount: float, category: str = "BOARD_GRANT", description: str = "Tutorial Bonus"):
    ...
    def update_calendar_round(self, round_num: int, track_name: str, circuit_file: str, total_laps: int, weather_profile: str, tier: Optional[int] = None, characteristic: str = "BALANCED") -> bool:
    ...
    def add_calendar_round(self, track_name: str, circuit_file: str, total_laps: int = 15, weather_profile: str = "DYNAMIC", tier: Optional[int] = None, characteristic: str = "BALANCED") -> int:
    ...
    def create_facility_node(
        ...
        with self.get_connection() as conn:
            ...
            """, (
                node_id, department, name, description, parent_id, tier, max_tier,
                base_cost, base_upkeep, staff_capacity, unlock_league_tier
            ))
    ...
    def delete_facility_node(self, node_id: str) -> bool:
        ...
        with self.get_connection() as conn:
            ...
            cur.execute("DELETE FROM team_equipment WHERE equipment_id IN (SELECT id FROM facility_equipment WHERE node_id = ?);", (node_id,))
        ...
        try:
            ...
            for s_num in [-1, 0]:
                for tier in range(1, 6):
                    cur.execute("SELECT id, name, color_hex, is_player FROM teams WHERE tier = ? ORDER BY reputation DESC, id ASC;", (tier,))
                    ...
                    for pos_idx, team in enumerate(shuffled_teams):
                        ...
                        cur.execute("SELECT id, name, is_player_driver, is_academy_driver FROM drivers WHERE team_id = ? ORDER BY id ASC;", (team["id"],))
                        team_drivers = [dict(r) for r in cur.fetchall()]
                        if not team_drivers:
                            team_drivers = [{"id": None, "name": f"Driver {team['name'][:4]}", "is_player_driver": 0, "is_academy_driver": 0}]
                        ...
                        for d_idx, d in enumerate(team_drivers[:2]):
                            ...
                            wins = max(0, min(tier_rounds // 2, (pts // 35) + rnd_state.randint(0, 1))) if pos_idx < 3 else 0
                            podiums = max(wins, min(tier_rounds, (pts // 18) + rnd_state.randint(0, 2))) if pos_idx < 6 else 0
                            ...
                            """, (
                                d["id"], d["name"], team["id"], team["name"], tier, s_num,
                                (pos_idx * 2 + d_idx + 1), pts, tier_rounds, wins, podiums,
                                d.get("is_player_driver", 0), d.get("is_academy_driver", 0)
                            ))

                    # Seed series_race_results for this tier and season
                    cur.execute("SELECT round, week, track_name FROM calendar WHERE tier = ? ORDER BY round ASC;", (tier,))
                    ...
                    for rnd in cal_rounds:
                        ...
                        for finish_pos, drv in enumerate(classified[:10], 1):
                            ...
                            """, (
                                tier, r_num, r_week, r_track, finish_pos, drv["name"],
                                drv["team_name"], drv["team_id"], drv["id"],
                                drv.get("is_academy_driver", 0), drv.get("is_player_driver", 0),
                                pts_award, s_num
                            ))
            ...
            if player_team_row:
                ...
                if cur.fetchone()[0] == 0:
                    ...
                    if alumni_candidate:
                        ...
                        """, (
                            p_id, a_did, a_dname, "Season 0 (2025)",
                            7, 1, 3, 42,
                            "PROMOTED_TIER2", 0, a_tid, a_tname, a_tier
                        ))
    ...
    def get_available_seasons(self) -> List[Dict[str, Any]]:
        ...
        with self.get_connection() as conn:
            cur = conn.cursor()
            cur.execute("SELECT DISTINCT season_num FROM season_history UNION SELECT DISTINCT season_num FROM driver_season_history ORDER BY season_num DESC;")
    ...
    def get_historical_season_races(self, season_num: int, tier: int) -> List[Dict[str, Any]]:
        ...
        with self.get_connection() as conn:
            ...
            if not rows and season_num == self.get_current_season_num():
                cur.execute("SELECT round as round_num, week, track_name, characteristic, is_completed FROM calendar WHERE tier = ? ORDER BY round ASC;", (tier,))
                cal = [dict(r) for r in cur.fetchall()]
                for c in cal:
                    if c["is_completed"]:
                        cur.execute("SELECT driver_name, team_name FROM series_race_results WHERE tier = ? AND round_num = ? AND season_num = ? AND position = 1;", (tier, c["round_num"], season_num))
                        w = cur.fetchone()
    ...
    def get_driver_profile(self, driver_id: Any, driver_type: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        with self.get_connection() as conn:
            ...
            if driver_type == "MARKET":
                ...
                if not row:
                    ...
                    if not m_row and not sp_row:
                        cur.execute("SELECT * FROM driver_season_history WHERE driver_id = ? ORDER BY season_num DESC LIMIT 1;", (driver_id,))
                        h_row = cur.fetchone()
            ...
            if driver_type == "SCOUT":
                ...
                cur.execute("SELECT MIN(position) FROM series_race_results WHERE driver_id = ? OR driver_name = ?;", (driver_id, d_dict["name"]))
                bf_row = cur.fetchone()
                if bf_row and bf_row[0] is not None:
                    best_fin = f"P{bf_row[0]}"
                elif total_wins > 0:
            ...
            if driver_type in ("MARKET", "SCOUT"):
                ...
                cur.execute("SELECT * FROM team_alumni WHERE driver_id = ? OR driver_name = ?;", (driver_id, d_dict["name"]))
    ...
    def _generate_driver_ai_analysis(self, p: Dict[str, Any]) -> Dict[str, str]:
        ...
        if starts == 0 and pot >= 80:
            ...
            arc = "ROOKIE PROSPECT"
            outlook = f"Has not yet contested an official championship season. Untested in open-wheel wheel-to-wheel combat."
        elif age <= 22 and pot >= 85:
            arc = "GENERATIONAL PRODIGY"
            outlook = f"Rapidly climbing the ranks. Projected to reach elite Tier 1 form within {max(1, 25 - age)} seasons."
        ...
        return {
            "trajectory_arc": arc,
            "scout_assessment": outlook,
            "alumni_insight": alumni_verdict
        }
    ...
    def record_team_alumni(
        ...
        with self.get_connection() as conn:
            cur = conn.cursor()
            cur.execute("SELECT id FROM team_alumni WHERE player_team_id = ? AND driver_id = ?;", (player_team_id, driver_id))
            row = cur.fetchone()
            if row:
                cur.execute("""
                UPDATE team_alumni
                ...
                WHERE id = ?;
                """, (
                    departure_reason, departure_season, starts_with_team,
                    wins_with_team, podiums_with_team, points_with_team,
                    current_team_id, current_team_name, current_tier, row[0]
                ))
            else:
    ...
    def get_team_profile(self, team_id: int) -> Optional[Dict[str, Any]]:
        ...
        with self.get_connection() as conn:
            ...
            # Titles
            cur.execute("SELECT COUNT(*) FROM season_history WHERE team_id = ? AND championship_position = 1 AND tier = 1;", (team_id,))
