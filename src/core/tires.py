...
class TireSet:
    ...
    def get_effective_grip(self, track_wetness: float = 0.0) -> float:
        ...
        if self.compound_name == "INTER":
            ...
            if track_wetness <= cap:
                ...
                slip_penalty = (track_wetness / max(0.01, cap)) * (0.05 if cap >= 0.18 else (0.09 if cap >= 0.14 else 0.15))
    ...
    def apply_wear_and_thermals(self, dist_travelled: float, track_length: float, 
                                dt: float, pace_multiplier: float = 1.0, 
                                cornering_g: float = 1.0, fuel_weight_kg: float = 50.0,
                                in_dirty_air: bool = False, track_wetness: float = 0.0):
        """
    ...
    def apply_wear(self, dt: float, pace_multiplier: float = 1.0, aggressive_driving: float = 1.0):
        self.wear_pct = min(100.0, self.wear_pct + (self.compound.deg_per_lap / 30.0) * dt * pace_multiplier * aggressive_driving)
