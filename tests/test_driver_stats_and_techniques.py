...
class TestDriverStatsAndTechniques(unittest.TestCase):
    ...
    def test_driving_style_inference(self):
        ...
        # Late braker
        d_brake = Driver(id=1, name="Braker", code="BRK", number=1, team_name="T1", color_rgb=(255, 0, 0),
                         braking=0.95, cornering=0.82, tire_management=0.80, overtaking=0.85, aggression=0.80)
        self.assertEqual(d_brake.driving_style, "LATE_BRAKER")

        # Smooth roller
        d_smooth = Driver(id=2, name="Smooth", code="SMO", number=2, team_name="T2", color_rgb=(0, 255, 0),
                          braking=0.80, cornering=0.96, tire_management=0.82, overtaking=0.80, aggression=0.60)
        self.assertEqual(d_smooth.driving_style, "SMOOTH_ROLLER")

        # Tire whisperer
        d_tire = Driver(id=3, name="Whisperer", code="WHS", number=3, team_name="T3", color_rgb=(0, 0, 255),
                        speed=0.82, braking=0.80, cornering=0.82, tire_management=0.96, consistency=0.94)
        self.assertEqual(d_tire.driving_style, "TIRE_WHISPERER")

        # Aggressive hunter
        d_hunt = Driver(id=4, name="Hunter", code="HNT", number=4, team_name="T4", color_rgb=(255, 255, 0),
                        braking=0.84, cornering=0.84, overtaking=0.94, aggression=0.88, consistency=0.75)
    ...
    def test_cornering_speed_and_technique_impact(self):
        ...
        drv_smooth = Driver(id=1, name="Smooth", code="SMO", number=1, team_name="T1", color_rgb=(0, 255, 0),
                            cornering=0.95, braking=0.80)
        drv_avg = Driver(id=2, name="Avg", code="AVG", number=2, team_name="T2", color_rgb=(255, 0, 0),
                         cornering=0.75, braking=0.80)
    ...
    def test_tire_management_and_wear_preservation(self):
        ...
        drv_whisperer = Driver(id=1, name="Whisperer", code="WHS", number=1, team_name="T1", color_rgb=(0, 0, 255),
                               tire_management=0.96)
        drv_hard = Driver(id=2, name="Aggressive", code="AGG", number=2, team_name="T2", color_rgb=(255, 0, 0),
                          tire_management=0.65, aggression=0.90, overtaking=0.92)
    ...
    def test_consistency_and_mistake_susceptibility(self):
        ...
        drv_calm = Driver(id=1, name="Calm", code="CLM", number=1, team_name="T1", color_rgb=(0, 255, 0), consistency=0.96)
        drv_erratic = Driver(id=2, name="Erratic", code="ERR", number=2, team_name="T2", color_rgb=(255, 0, 0), consistency=0.65)
        ...
        self.assertGreater(risk_erratic, risk_calm * 2.0, "Erratic driver should have over double the mistake risk under pressure")
    ...
    def test_wet_weather_skill_impact(self):
        ...
        drv_rainmaster = Driver(id=1, name="RainMaster", code="RNM", number=1, team_name="T1", color_rgb=(0, 200, 255),
                                wet_skill=0.95, speed=0.80)
        drv_novice = Driver(id=2, name="RainNovice", code="NVC", number=2, team_name="T2", color_rgb=(100, 100, 100),
                            wet_skill=0.60, speed=0.80)
        ...
        wetness = 0.70
        for _ in range(5):
            car_rainmaster.update_physics(0.1, self.circuit, self.rc, track_wetness=wetness, car_ahead=None, car_behind=None)
            car_novice.update_physics(0.1, self.circuit, self.rc, track_wetness=wetness, car_ahead=None, car_behind=None)
