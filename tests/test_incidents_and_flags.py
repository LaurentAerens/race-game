...
from src.core.race_control import FlagStatus, RaceControl
...
class TestIncidentsAndFlags(unittest.TestCase):
    ...
    def test_off_track_incident_and_safe_rejoin(self):
        ...
        car.update_physics(
            dt=0.5, circuit=self.circuit, race_control=rc, track_wetness=0.0, car_ahead=None, car_behind=None
        )
        ...
        car.update_physics(
            ...
            track_wetness=0.0,
            car_ahead=None,
            car_behind=rival_behind,
            all_cars=[car, rival_behind],
        )
    ...
    def test_sector_yellow_flag_speed_and_overtake_ban(self):
        ...
        car.update_physics(
            dt=0.2, circuit=self.circuit, race_control=rc, track_wetness=0.0, car_ahead=car_ahead, car_behind=None
        )
        self.assertFalse(car.is_overtaking)
    ...
    def test_driver_bunching_and_traffic_risk(self):
        ...
        solo_bunch = sum(
            1 for oc in all_cars if oc is not car_solo and not oc.is_broken and (abs(oc.s - car_solo.s) < 25.0)
        )
        # Pack car has 2 nearby cars
        pack_bunch = sum(
            1
            for oc in all_cars
            if oc is not car_pack_leader and not oc.is_broken and (abs(oc.s - car_pack_leader.s) < 25.0)
        )
    ...
    def test_simulation_flag_neutralization_triggers_on_all_cars(self):
        ...
        sim = Simulation(
            circuit=self.circuit,
            driver_car_pairs=[(player_d1, self.attrs), (player_d2, self.attrs)],
            total_laps=10,
            league_tier=2,
        )
        for car in sim.cars:
    ...
    def test_sector_yellow_flag_conservative_mode_only_in_sector(self):
        ...
        car.update_physics(
            dt=0.1, circuit=self.circuit, race_control=rc, track_wetness=0.0, car_ahead=None, car_behind=None
        )
        self.assertFalse(car.is_mode_locked)
