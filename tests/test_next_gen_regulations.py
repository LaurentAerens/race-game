...
class TestNextGenRegulations(unittest.TestCase):
    ...
    def test_weekly_progression_and_telemetry_dampening(self):
        ...
        with self.db.get_connection() as conn:
            cur = conn.cursor()
            cur.execute("SELECT id, knowledge_max FROM car_components WHERE team_id = ? AND car_slot = 1 LIMIT 1;", (self.team_id,))
            comp = cur.fetchone()
    ...
    def test_week_9_regulations_evaluation(self):
        ...
        self.assertIn(reg2["upcoming_package"], ["AERO_SHAKEUP", "MECHANICAL_TWEAKS", "POWERTRAIN_DIRECTIVE", "MAJOR_OVERHAUL"])
        ...
        self.assertIn(reg4["upcoming_package"], ["MECHANICAL_TWEAKS", "AERO_SHAKEUP", "POWERTRAIN_DIRECTIVE", "MAJOR_OVERHAUL"])
    ...
    def test_in_season_port_back_upgrade(self):
        ...
        with self.db.get_connection() as conn:
            cur = conn.cursor()
            cur.execute("UPDATE teams SET next_gen_rnd_points = 50000.0, chassis_perf_boost = 0.0, chassis_rel_boost = 0.0 WHERE id = ?;", (self.team_id,))
            cur.execute("INSERT OR REPLACE INTO season_regulations (season_num, tier, is_announced, upcoming_package, port_back_used) VALUES (1, 3, 1, 'STATUS_QUO', 0);")
            conn.commit()
    ...
    def test_season_rollover_resets_and_boosts(self):
        ...
        with self.db.get_connection() as conn:
            cur = conn.cursor()
            cur.execute("UPDATE teams SET next_gen_rnd_points = 75000.0, chassis_perf_boost = 5.0 WHERE id = ?;", (self.team_id,))
            cur.execute("UPDATE car_components SET performance = 120.0, generation = 3 WHERE team_id = ? AND category = 'FRONT_WING';", (self.team_id,))
            cur.execute("UPDATE car_components SET performance = 110.0, generation = 2 WHERE team_id = ? AND category = 'BRAKES';", (self.team_id,))
            cur.execute("INSERT OR REPLACE INTO season_regulations (season_num, tier, upcoming_package) VALUES (1, 3, 'AERO_SHAKEUP');")
            conn.commit()
        ...
        with self.db.get_connection() as conn:
            cur = conn.cursor()
            cur.execute("SELECT performance, generation FROM car_components WHERE team_id = ? AND category = 'FRONT_WING' AND car_slot = 1;", (self.team_id,))
            fw = cur.fetchone()
            ...
            cur.execute("SELECT performance, generation FROM car_components WHERE team_id = ? AND category = 'BRAKES' AND car_slot = 1;", (self.team_id,))
            brk = cur.fetchone()
            ...
            cur.execute("SELECT chassis_perf_boost, chassis_tire_preservation_base FROM teams WHERE id = ?;", (self.team_id,))
            t_row = cur.fetchone()
