...
class TestGameplayFixes(unittest.TestCase):
    ...
    def test_qualifying_shootout_click_coordinates(self):
        driver1 = Driver(
            id=1, name="Driver 1", code="D1", number=1, team_name="Team 1", color_rgb=(255, 0, 0), is_player=True
        )
        driver2 = Driver(
            id=2, name="Driver 2", code="D2", number=2, team_name="Team 2", color_rgb=(0, 0, 255), is_player=False
        )
        attrs = CarAttributes()
        pairs = [(driver1, attrs), (driver2, attrs)]

        mgr = RaceWeekendManager(league_tier=3, track_metadata={"track_name": "Emerald Ring"}, total_laps_base=15)
        screen = RaceWeekendScreen(
            1280,
            720,
            manager=mgr,
    ...
    def test_broadcast_header_chequered_flag(self):
        ...
        driver = Driver(
            id=1, name="Leader", code="LEA", number=1, team_name="Team", color_rgb=(255, 200, 0), is_player=True
        )
